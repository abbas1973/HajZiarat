...
namespace Utilities.Files
{
    public static class SaveFileExtension
    {
        ...
        /// <param name="absolutePath">آدرس فیزیکی روت پروژه (بیرون wwwroot(</param>
        /// <returns>نام نهایی فایل ذخیره شده</returns>
        public static async Task<SaveFileResult> SaveFile(
            this IFormFile file,
            string relativePath,
            string absolutePath = null,
            FileFormat? type = null,
            string FileName = null)
        {
            try
            {
                #region بررسی فرمت فایل
                if (file == null)
                    return new SaveFileResult(false, "فایل موجود نیست!");

                if (relativePath == null)
                    return new SaveFileResult(false, "مسیر ذخیره فایل را مشخص کنید!");
                ...
                #region ایجاد مسیر فایل
                if(absolutePath == null)
                    absolutePath =  Path.Combine(Directory.GetCurrentDirectory(), "wwwroot");

                relativePath = relativePath.Trim('/').Replace("/", "\\");
                string dir = Path.Combine(absolutePath, relativePath);
                ...
                return new SaveFileResult(true, "ذخیره فایل با موفقیت انجام شد.", FileName, fullPath);
            }
            catch (Exception ex)
            {
